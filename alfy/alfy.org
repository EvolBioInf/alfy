...
position. Additionally the match length cannot be zero, as that means
no match exists in the current position.
#+end_export
...
close it and append the last interval to the slice of windows.
#+end_export
...
#+end_src
#+begin_export latex
If the window is homologous we create a new window and populate it
with the current left and right border and current calculated
score. Since slices are reference types we store the score with
copy. This ensures we store the result without affecting the scores of
the follow up windows.
#+end_export
...
intervals. We start by creating a final structure
#+end_export
#+begin_src go << Merge adjacent windows, Ch. \ref{ch:alf}>>=
  ...
  //<<Merge overlapping windows, Ch. \ref{ch:alf}>>
  //<<Calculate nm per bp, Ch. \ref{ch:alf}>>
#+end_src
#+begin_src go << Types, Ch. \ref{ch:alf}>>=
  type Summary struct {
	  start int
...
#+begin_src go << Imports, Ch. \ref{ch:alf}>>=
  "slices"
#+end_src
#+begin_src go << Merge overlapping windows, Ch. \ref{ch:alf}>>=
  var border int
  for a, sum := range summary {
	  if a == 0 {
		  border = sum.end
	  } else {
		  sum.start = border +1
...
#+end_src
#+begin_src go <<Calculate nm per bp, Ch. \ref{ch:alf}>>=
  for _, sum := range summary {
	  var len int
	  len = sum.end-sum.start+1
	  sum.nm= sum.nm/float64(len)
//	  fmt.Println("start",sum.start+1,"end",sum.end+1,"nm",sum.nm,"winner",sum.label)
  }
#+end_src
#+begin_export latex
We then prepare to write the output table. We first print the header,
and then parse the subject IDs names and print them out.
#+end_export
