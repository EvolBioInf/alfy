...
reached the end of the sequence, if yes we store the last
window. Otherwise we compare the annotation of the current window with
the next one.
#+end_export
...
maxSbjct variable. We also set the current slice of IDs as the current one.
#+end_export
...
update the sum score of the window.
#+end_export
