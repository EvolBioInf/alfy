...
right borders of each window, the annotation of the subject ID as well
as two types of scores, the sum of the window score and the score per
bp. We start by constructing a structure type window, which will
contain all of these variables.
#+end_export
#+begin_src go <<Types, Ch. \ref{ch:alf}>>=
  type Window struct{
	  start int
...
#+end_src
#+begin_export latex
To check for adjacent windows we start by initializing the window type
variable. Then we iterate over the length of the maxSbjct, which
...
variable p, and the score by variable s. To check whether the windows
can be merged we check whether the slice of IDs, between the current
and the next window, are identical. If not we close the window. We
also close the last window when reaching the end of the sequence.
#+end_export
...
ID. Since the sliding window analysis is done by sliding one step to
the right, we know that the right border of a given window,
window\textsubscript{`$n$}, starts $n$ positions to the right of the
window size.

The slice of ID of each window corresponds to the slices stored in the
maxSbjct variable. We set the current slice of IDs as the current one,
and the next set of subject IDs as the next one.
#+end_export
...
#+end_src
#+begin_export latex
If the two slices of subject IDs are identical, we extend the right
border by one position, and update the score to also include the new
window score.
#+end_export
...
score per base pair. Since so far we have been dealing with integers
we need to convert these to floats. We then append the results to the
array of previously saved windows and break out of the loop. The left
border of the window is given by the variable p. After appending the
current window to the slice of closed windows, we update the left
border of the new window, and set the current window score to 0.
#+end_export
...
reached the end. We update the left border of the window, and its
existing score. After calculating the score per base pair we append
the last window to the slice of closed windows and break out of the loop.
#+end_export
...
the subject IDs, we want to directly print the subject
names. Afterwards we print the summary of the intervals.
#+end_export
...
the last substring, delimited by a dot, and slice the string up until
that point.
#+end_export
...
We have now finished writing alfy, and move onwards to the testing.

The program to test alfy has hooks for imports and the testing logic.
#+end_export
...
#+end_src
#+begin_export latex
In the first test, we test the program with a window size of 5, half
the lenght of the test sequence..
#+end_export
...
#+end_src
#+begin_export latex
In a second test, we test with a window size of 10, the length of the
testing sequence.
#+end_export
#+begin_src go <<Construct tests, Ch. \ref{ch:alf}>>=
  w = "10"
...
#+end_src
#+begin_export latex
We run a test and compare the output with the one we want, which are
stored in the files r1.txt and r2.txt.
#+end_export
