...
borders and respective scores. To check whether the windows can be
merged we check whether the slice of IDs, between the current and the
next window, are identical. If not we close the window. We also close
the last window when reaching the end of the sequence.
#+end_export
...
#+end_src
#+begin_export latex
We import slices to compare the slices of subject IDs.
#+end_export
#+begin_src go <<Imports, Ch. \ref{ch:alf}>>=
  "slices"
#+end_src
#+begin_export latex
Each window consists of a left border, the start, a right border, the
end, a score and a slice of IDs. Since the sliding window analysis is
done by sliding one step to the right, we know that the left border of
a given window, window\textsubscript{`$n$}, starts $n$ positions to
the right of the first nucleotide. Likewise, the right border, ends
$n$ positions to the right of the size of the window.
...
array of previously saved windows and break out of the loop.
#+end_export
...
reached the end. We append the last window and break out of the loop.
#+end_export
...
#+end_src
#+begin_src go <<Print scores, Ch. \ref{ch:alf}>>=
