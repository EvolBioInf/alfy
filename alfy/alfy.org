...
#+end_src
#+begin_export latex
The window structure holds five parameters. The left and right border
of the interval, the length of the fragment of each subject ID found
in a given window and the corresponding slice of subject IDs, and the
total number of mismatches found in the entire window.
#+end_export
#+begin_src go <<Types, Ch. \ref{ch:alf}>>=
  type Window struct{
	  start int
...
#+end_src
#+begin_export latex
When opening the first window we set a counter of mismatches, $nm$, to
1, and the left and right border to 0. Then, while the current
position, $r$, is smaller than the window size, we count the number of
base pairs each given subject ID covers, and the number of mismatches
in the query. Since match lengths decrease at every position, we have
found new match if the match length remains or increases despite
walking one position to the left. This marks the presence of a
mismatch.
#+end_export
...
the matches. We also check if the intervals are homologous or
non-homolgous, query unique. If the first opened window is longer or
equal to the matches length, we only evaluate their homology.
#+end_export
...
homology signal. If the number of mismatches is higher than the
calculated threshold, then this is a query unique region.
#+end_export 
#+begin_src go <<Check if homologous or unique, Ch. \ref{ch:alf}>>=
  if nm < t {
	  //<<Open homologous interval, Ch. \ref{ch:alf}>>
  } else if nm >= t {
	  //<<Open query unique interval, Ch. \ref{ch:alf}>>
  }
#+end_src
#+begin_export latex
If the window contains signs of homology, we note that the interval is
``homologous''. Then we create a new window and populate it with the
current left and right border and current calculated score. Since
slices are reference types we store the score with copy. This ensures
we store the result without affecting the scores of the follow up
windows. We want the right interval border to be inclusive, so we
subtract 1. We also store the window in a slice of windows.
#+end_export
...
#+end_src
#+begin_export latex
If the interval we are dealing with is query unique, we set a note
that the interval is ``unique''. We store the corrent interval score,
again with copy, and borders in the slice of windows, together with
the note and the number of mistmatches. We want the right interval
border to be inclusive, so we subtract 1.
#+end_export
#+begin_src go <<Open query unique interval, Ch. \ref{ch:alf}>>=
  window = new(Window)
  window.score = make([]int, len(score))
  copy(window.score,score)
  window.start = l
...
and the subject ID score. Then we shift the window by one base pair.
#+end_export
...
#+end_src
#+begin_export latex
\subsection{Label and merge adjacent windows}
After the sliding window analysis, the next step is to label each
window with the winner subject IDs. We weigth the label of the window
with the previously calculated score, which reflects the number of
base pairs, for a given windows, that the subject ID
encompasses. Therefore, subject IDs with longer matches will be
favoured.

We start by iterating over the stored slices of windows, and weight
the subject IDs according to their scores. If a window is marked as
homologous, then the ID, or IDs, with the highest scores are called
the winners. If the window is marked as unique, then no subject ID is
to be called as the winner.
#+end_export
#+begin_src go <<Label windows winner, Ch. \ref{ch:alf}>>=
  for _, win := range windows {
    ...
    //<<Check printed windows, Ch. \ref{ch:alf}>>

#+end_src
#+begin_src go <<Check printed windows, Ch. \ref{ch:alf}>>=
  for _, win := range windows {
	  for _, w := range win {
...
if two adjacent windows share the same winner, or are considered
unique we merge them and update the left and right border of the
intervals.
#+end_export
#+begin_src go <<Merge adjacent windows, Ch. \ref{ch:alf}>>=
#+end_src
#+begin_export latex
\textbf{To be redone}\\ From here on onwards, it needs to be rewritten
to match the new sliding window analysis. Code chunks currently being
ignored.
#+end_export
