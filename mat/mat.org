...
and note the label of the current subject sequence, $i$. If $\ell$ is
equal to $\mbox{ml}[j]$ we append the label of the current subject
sequence to the same position.
#+end_export
...
Algorithm~\ref{alg:int}. At each position, the match to the left is
extended unless we are at the start of a new match. When extending the
match, we update the match length at the current position and the
subject ID. If, when extending the match, we encounter a new match
with the same length we merge the subject IDs.
#+end_export
