...
#+end_src
#+begin_export latex
To perform the union of the two slices we first create a dictionary
the length of the first slice. Then we store the elements of the first
slice. Afterwards we check the elements of the first slice, and if an
element of the second slice is not present we append to it. At the end
we return the first slice now also containing all the new elements of
the second slice.
#+end_export
#+begin_src go <<Functions, Pa. \ref{pa:mat}>>=
