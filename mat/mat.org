...
#+end_src
#+begin_export latex
To perform the union of the two slices we construct the function
\ty{union}. First we ensure that slice a is the smallest. Then we
index the smallest slice, a, and construct a dictionary of all values
it contains. Then we calculate the elemements of the union between a
and b. At the end we return a.
#+end_export
#+begin_src go <<Functions, Pa. \ref{pa:mat}>>=
  func union(a, b []int) []int {
	  //<<Ensure a less than b, Pa. \ref{pa:mat}>>
...
#+end_src
#+begin_export latex
If the slice a is longer than slice b, we swap the names. This ensures
that slice a is always the smallest one.
#+end_export
#+begin_src go <<Ensure a less than b, Pa. \ref{pa:mat}>>=
  if len(a) > len(b) {
	  tmp := a
	  a = b
	  b = tmp
...
#+end_src
#+begin_export latex
To calculate the union of a and b, we iterate over the elements of b,
and if a given element is not present in the dictionary previously
constructed we append to it.
#+end_export
