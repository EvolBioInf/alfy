...
#+end_src
#+begin_export latex
We construct short queries and subjects. To get matches, we reduce the
nucloetide alphabet largely to $\{\ty{A},\ty{T}\}$. Our first query is 
\ty{AAATAATATT}, or second distinguished by one mutation to an
extraneous nucleotide, \ty{G}
#+end_export
...
#+end_src
#+begin_export latex
Similarly, we construct two short subjects. However, here we do stick
to the \ty{AT} alphabet.
#+end_export
