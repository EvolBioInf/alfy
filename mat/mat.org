...
#+end_src
#+begin_export latex
We also contruct the function \ty{Store} to store the arrays of match
lenghts and subject IDs that are returned after the go routine. The
\ty{Store} function is extremely similar to the \ty{Update} function
excepts it run across every nucleotide without skipping the length of
the match. Additionally, if the two match lengths are identical it
keeps the entire slice of subject IDs and not just the first one.
#+end_export
#+begin_src go <<Functions, Pa. \ref{pa:mat}>>=
