...
equal to $\mbox{ml}[j]$ we are looking at a new interval with the same
...
subject ID. We append the new subject ID to the slice of IDs being
extended. If, when extending the match, we encounter a new match with
the same length we merge the subject IDs.
#+end_export
