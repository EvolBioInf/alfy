...
!array of subject labels, whether or not the query is a reverse
!strand, and whether or not the program should step through all
!possible match start positions. It then updates the arrays of match
!lengths and subject labels.
...
$j$, by at least one position. If the user requested skipping instead
of stepping, we skip the start of the next match just beyond the
terminal mismatch. The matches are saved in the mirrored arrays.
#+end_export
...
check if the arrays need to be updated. Then advance by one position.
#+end_export
#+begin_src go <<Functions, Pa. \ref{pa:mat}>>=
...
#+end_src
#+begin_export latex
If the match in the data array, $\mbox{dml}[p]$, is greater than
$\mbox{ml}[p]$, we update $\mbox{ml}[p]$ and note the label of the
current subject sequence, $dsu[p][0]$. If $\mbox{dml}[p]$ is equal to
$\mbox{ml}[p]$ we are looking at a new interval with the same
length. In this case, we perform the union between the subject IDs
carried by the previous match and the current one.
#+end_export
...
#+end_src
#+begin_export latex
To perform the union of the two slices $a$ and $b$, we construct the
function \ty{union}. First we declare a slice for the union, $u$. Then
we ensure that slice $a$ is the smaller of the two and construct a
dictionary of its values. Then we calculate the union between $a$ and
$b$ and return it.
#+end_export
#+begin_src go <<Functions, Pa. \ref{pa:mat}>>=
  func union(a, b []int) []int {
	  u := []int{}
	  //<<Ensure $|a| < |b|$, Pa. \ref{pa:mat}>>
	  dic := make(map[int]bool)
	  for _, e := range a {
		  dic[e] = true
	  }
	  //<<Calculate union of a and b, Pa. \ref{pa:mat}>>
	  return u
  }
#+end_src
#+begin_export latex
If slice $a$ is longer than slice $b$, we swap the names. This
ensures that slice $a$ is always the smallest one.
#+end_export
#+begin_src go <<Ensure $|a| < |b|$, Pa. \ref{pa:mat}>>=
  if len(a) > len(b) {
	  tmp := a
	  a = b
	  b = tmp
...
#+end_src
#+begin_export latex
To calculate the union of $a$ and $b$, we copy the elements of $a$
into $u$, iterate over the elements of $b$, and if a given element is
not present in the dictionary previously constructed from $a$, we
append it to $u$.
#+end_export
#+begin_src go <<Calculate union of a and b, Pa. \ref{pa:mat}>>=
  for _, e := range a {
	  u = append(u, e)
  }
  for _, e := range b {
...
false before we compare queries and subjects to do the actual
testing. The bolean \ty{step} indicates whether the program should
step through all possible match start positions or skip the match. We
start by setting it to false, meaning we skip the match. Afterwards we
set it to positive to evaluate the stepping on the forward strand.
#+end_export
#+begin_src go <<Test analysis of forward strand, Pa. \ref{pa:mat}>>=
  ...
  step = true
  //<<Construct wanted step results, Pa. \ref{pa:mat}>>
  //<<Compare queries and subjects, Pa. \ref{pa:mat}>>
#+end_src
#+begin_export latex
...
#+end_src
#+begin_export latex
We also construct the match lengths and the subject labels we want
when using the stepping model.
#+end_export
#+begin_src go <<Construct wanted step results, Pa. \ref{pa:mat}>>=
  wml = [][]int{
...
construct them, toggle \ty{rev} to true and \ty{step} to false and
compare the queries and subjects.
#+end_export
