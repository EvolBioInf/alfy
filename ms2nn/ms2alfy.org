...
header, and iterate over the input to store the trees. From the stored
trees we find the query's neighbors. Trees may have the same set of
...
its extent, construct the tree from it, and store that. Since the
input only contains tree extents, we track the tree start from
position 1 onward.
#+end_export
#+begin_src go <<Store tree, Pr. \ref{pr:m2a}>>=
  ...
  if len(line) > 0 && line[0] == '[' {
	  tree := new(Tree)
	  //<<Extract tree string, Pr. \ref{pr:m2a}>>
	  //<<Extract extent of tree, Pr. \ref{pr:m2a}>>
	  //<<Construct tree, Pr. \ref{pr:m2a}>>
...
string.
#+end_export
#+begin_src go <<Extract tree string, Pr. \ref{pr:m2a}>>=
...
#+end_export
#+begin_src go <<Extract extent of tree, Pr. \ref{pr:m2a}>>=
...
#+end_export
#+begin_src go <<Construct tree, Pr. \ref{pr:m2a}>>=
  tree.start = start
...
#+end_export
#+begin_src go <<Merge trees, Pr. \ref{pr:m2a}>>=
  //<<Sort neighbors, Pr. \ref{pr:m2a}>>=
  //<<Merge trees with identical neighbors, Pr. \ref{pr:m2a}>>
#+end_src
#+begin_export latex
We sort the neighbors by calling a library function for sorting
strings.
#+end_export
#+begin_src go <<Sort neighbors, Pr. \ref{pr:m2a}>>=
  for _, tree := range trees {
...
neighbor, we merge them by extending the end position of the left hand
neighbor.
#+end_export
#+begin_src go <<Merge trees with identical neighbors, Pr. \ref{pr:m2a}>>=
  c := 0
  for i := 1; i < len(trees); i++ {
	  if slices.Equal(trees[c].neighbors, trees[i].neighbors) {
		  fmt.Println("equal: ", trees[c].neighbors, trees[i].neighbors)
