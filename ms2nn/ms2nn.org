...
#+end_src
#+begin_export latex
Apart from the obligatory version, \ty{-v}, we declare three flags,
the query, \ty{-q}, merge by topology (\ty{-t}), and merge by distance
(\ty{-d}).
#+end_export
...
#+end_src
#+begin_export latex
We parse the flags and respond to the version flag, the query flag,
and the merge flags.
#+end_export
#+begin_src go <<Parse flags, Pr. \ref{pr:m2n}>>=
  ...
  //<<Respond to merge flags, Pr. \ref{pr:m2n}>>
#+end_src
#+begin_export latex
...
#+end_src
#+begin_export latex
Switching on both merge flags may lead to different results, depending
on the order in which the merges are carried out. It is also probably
an error. So in that case we kindly ask the user to chose at most one
of the merge flags.
#+end_export
#+begin_src go <<Respond to merge flags, Pr. \ref{pr:m2n}>>=
  if *flagT && *flagD {
...
as argument the query and the two merge flags.
#+end_export
...
#+end_src
#+begin_export latex
Inside the function \ty{parse}, we retrieve the arguments, print it as
the header, and iterate over the input to store the trees. Since the
input only contains a tree's extent, we use the variable \ty{start} to
track their starting positions. From the stored trees we find the
query's neighbors. Trees may have the same set of neighbors, in which
case we merge their intervals, before we print the neighbors.
#+end_export
...
#+end_src
#+begin_export latex
We retrieve the three aguments query, merge by topology, and merge by
distance by type assertion.
#+end_export
...
#+begin_src go <<Merge trees, Pr. \ref{pr:m2n}>>=
  ...
  if mergeT {
	  ...
	  //<<Merge trees with equidistant neighbors, Pr. \ref{pr:m2n}>>
  }
#+end_src
#+begin_export latex
