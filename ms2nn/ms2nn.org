...
relevant, as its second argument is the sequence length.  Unless the
recombination rate is zero, trees start with their extent along this
sequence in square brackets. They always end with a semi colon. The
...
we exceed the end of the tree. The position we have now reached is
just beyond the desired one, and identical to the start position for
the next round.
#+end_export
