...
DNA sequences are analyzed with \ty{alfy} and the output is stored in
the file \ty{ms.alfy}. In a second analysis strand the original
haplotype sample in \ty{ms.out} is converted to the ``true'' output
with the program \ty{ms2nn} and the result is stored in
\ty{ms.nn}. Now the contents of \ty{ms.alfy} and \ty{ms.nn} can be
compared to assess the performance of \ty{alfy}.
...
mutation and recombination rates $\theta=\rho=100$:
\begin{verbatim}
...
\end{verbatim}
The first line gives the \ty{ms} command, of which the sample size and
the sequence length of the \ty{-r} flag are relevant for the
subsequent analysis.  Unless the recombination rate is zero, trees
start with their extent along this sequence in square brackets. They
always end with a semi colon. The mutations, or segregating sites,
along the sequence are given for the interval $(0,1)$, which we
convert to sequence positions by multiplying with the sequence
length. The haplotypes of zeros and ones, finally, are the basis for
calculating distances to the query.
...
data we find the query's neighbors per tree. Trees may have identical
neighbors, in which case we merge their intervals, before we print the
neighbors.
#+end_export
...
the positions of mutations, and the haplotypes. We also declare a
variable for the tree start position, which we initialize to the
position implied by \ty{ms}, 1. In the zero-based world of Go this
means 0.
#+end_export
...
\item a row of positions of mutations starting with ``p'', in which
  case we store them
\item a row of polymorphisms starting with ``0'' or ``1'', in which case
  we store the resulting haplotype
\end{enumerate}
#+end_export
...
ms 5 1 -t 100 -r 100 10000 -T
\end{verbatim}
of which we need the sample size, 5, the sequence length, 10,000, and
require tree printing, \ty{-T}. Since tree printing is necessary for
...
array. The tree also contains neighbors and mismatches with respect to
a query. The tree itself is stored as its root.
#+end_export
...
#+end_src
#+begin_export latex
To store a tree, we extract it together with
its extent, and construct it.
#+end_export
...
closing square bracket. If the tree has an extent, the second entry in
the resulting array is the tree string, otherwise it's the first
entry.
#+end_export
...
intersects the tree. We make sure that we do not move beyond the end
of the slice of positions.
#+end_export
...
of the tree to $(0,1)$---like we did with the start---and walk right
until we exceed the end of the tree. The position we have now reached
is just beyond the desired one, and identical to the start position
for the next round.
#+end_export
...
which case we signal this by setting the number of mismatches per site
to the nonsensical -1. Otherwise we simply divide the mismatches by
the tree extent.
#+end_export
