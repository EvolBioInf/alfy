...
\caption{Finding the longest matches between a query sequence and a set of subject sequences.}\label{alg:alf}
  \begin{algorithmic}[1]
    ...
    subjects containing the longest matches, $\mbox{su}$; these come
    initialized to zero
...
For a given query sequence of length $m$, $q$, our algorithm iterates
over the subjects and first converts a given subject to its enhanced
suffix array, $\mbox{esa}$ (line 2). Next it iterates across $q$ and
calculates an array of longest match lengths, $\mbox{ml}$, and an
array of subjects inducing these lengths, $\mbox{su}$.
...
$i$ (lines 5--8). Then the active point of the analysis,$j$, skips to
the first position beyond the terminating mismatch, $j\leftarrow
1+\ell+1$ (line 9) and the analysis is repeated.
