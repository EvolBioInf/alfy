...
Algorithm~\ref{alg:prep} shows a sketch of the method at the heart of
our reimplementation of Alfy reduced to a single query input. This
procedure is simply iterated to deal with multiple
queries. Algorithm~\ref{alg:prep} is essentially the algorithm used in
...
\begin{algorithm}
\caption{Finding the longest matches between a query sequence and a set of subject sequences.}\label{alg:prep}
  \begin{algorithmic}[1]
...
\begin{algorithm}
  \caption{$\langle\mbox{match}\rangle$ from
    Algorithm~\ref{alg:prep}.}\label{alg:mat}
  \begin{algorithmic}[1]
    \STATE $j\leftarrow 1$
    \WHILE{$j\le m$}
...
\begin{algorithm}
  \caption{$\langle\mbox{interpolate}\rangle$ from
    Algorithm~\ref{alg:prep}.}\label{alg:int}
  \begin{algorithmic}[1]
...
phase shown in lines 1 and 2 of Table~\ref{tab:alg}.
  ...
  \begin{table}
    \caption{Tracing Algorithm~\ref{alg:prep} for $q_1=\ty{AAATAATAGT}$.}\label{tab:alg}
    \begin{center}
...
for targets and neighbors in \ty{makeNeiDb}. The output file of
\ty{prepAlfy} contains two types of headers followed by four numerical
columns. The first header, marked by \#, denotes the name of the
query, the filename, the second header marked by \textgreater, denotes
the name of the sequence, its length and a map between the subject IDs
and their names. After the header we describe the intervals of match
lengths. Each row describes the left and right border of the interval,
the match length at the first position of the interval and the slice
of subjectIDs that describe where the match length was
found. Figure~\ref{fig:out} shows an example for two short queries
that were compared to two subjects.
...
The map of subject IDs is a legend of key/value pairs
\begin{verbatim}
subject_id=subject_file
\end{verbatim}
We print subject identifiers rather than names in column 2, to save
space---identifiers are usually shorter than file names. As shown in
Figure~\ref{fig:out}, each table is understood to cover every position
in the respective query.

To avoid printing the match length and subject identifiers at every
position of the sequence, \ty{prepAlfy} summarizes them into a set of
intervals. Each row denotes an interval where the subject IDs remained
identical. The third column of the output file keeps track of the
match length at the first position of the
interval. Algorithm~\ref{alg:out} summarizes the procedure to generate
the output of \ty{prepAlfy}. This consists of two for-loops. In the
first (lines 1--6), we construct a map of the subject IDs and names of
the subjects found in the query. Then we print that map as part of the
table header (line 7). In the second for-loop (lines 8--10) we iterate
over the array of match lengths, and if the subject ID is the same we
extend the interval. If it differs we close the interval.

\begin{algorithm}
  \caption{$\langle\mbox{output}\rangle$ from
    Algorithm~\ref{alg:prep}.}\label{alg:out}
  \begin{algorithmic}[1]
    \REQUIRE queryName
    \REQUIRE $m$, query length
    \REQUIRE $\mbox{ml}$, array of $m$ match lengths
    \REQUIRE su, array of $m$ subject identifiers
    \REQUIRE subjectNames, array of subject names
    \ENSURE subjectsFound, map of subject IDs and names
    \ENSURE Table of match lengths and subject IDs with header
    consisting of queryName and ID/name pairs of the subjects found
    \ENSURE left, array of positions describing the left border of the intervals
    \ENSURE right, array of positions describing the right border of the intervals
    \FOR{$i\leftarrow 1$ \TO $m$}
    \STATE $s\leftarrow \mbox{su}[i]$
    \IF{$\mbox{subjectsFound}[s]=\ty{""}$}
    \STATE $\mbox{subjectsFound}[s]\leftarrow \mbox{subjectNames}[s]$
    \ENDIF
    \ENDFOR
    \STATE $\mbox{print \#queryName, subjectsFound}$
    \FOR{$i\leftarrow$ \TO $m$}
    \STATE $\mbox{left}\leftarrow i$ 
    \IF{$\mbox{su[i]}$ == $\mbox{su[i-1]} || \mbox{ml[i]} > \mbox{ml[i-1]-1}$}
    \STATE $\mbox{right}\leftarrow i$ 
    \STATE $\mbox{ml}\leftarrow \mbox{ml}-1$ 
    \ENDIF
    \STATE $\mbox{print}$ $\mbox{start}[i]$, $\mbox{end}[i]$, $\mbox{ml}[i]$, $\mbox{su}[i]$
    \ENDFOR     
  \end{algorithmic}
\end{algorithm}

The \ty{alfy} program then performs a sliding window analysis on the
matching results to calculate the homology between the query and the
subject sequences. Regions can have one of the following three
denominations. They can be unique, in which case no homology is shared
between the query region and the subjects, they can have a strong
homology to one subject sequence, or they can have a weak homology to
one or more subject sequences in this case they are marked as
non-homologous regions.

By annotating the unique regions in the query, these regions can be
quickly skipped over during the sliding window analysis, as we know
\textit{a priori} that no homology is to be found.

Algorithm~\ref{alg:alf} shows the sliding window algorithm.
\begin{algorithm}
  \caption{Calculating the homology between from a set of match
    lengths and subject identifiers.}\label{alg:alf}
\end{algorithm}

The output from \ty{alfy} is similar to the one from the previous
version, shown in Figure~\ref{fig:alfo}. Its comprised by a four column
table with the start and end coordinates of the homologous region in
the query, the percentage of the region that is homologous and the
name of the subject it is homologous to.

\begin{figure}[h]
\caption{The output of \ty{alfy} for a window of 5bp.}
\label{fig:alfo}
\begin{center}
  \footnotesize
\begin{tabular}{|c|c|}
...
then carries out lines 3--10 of Algorithm~\ref{alg:prep} and returns
