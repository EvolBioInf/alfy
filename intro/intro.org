...
Algorithm~\ref{alg:out} summarizes the procedure to generate the
output of \ty{prepAlfy}. This consists of two for-loops. In the first
(lines 1--6), we construct a map of the subject IDs and names of the
subjects found in the query. Then we print that map as part of the
table header (line 7). In the second for-loop (lines 8--10) we print
the pairs of match lengths and subject identifiers, one for each query
position.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \FOR{$i\leftarrow$ \TO $m$}
       \STATE $\mbox{print}$ $\mbox{ml}[i]$, $\mbox{su}[i]$
    \ENDFOR
  \end{algorithmic}
\end{algorithm}
...
The output from \ty{alfy} is identical to the one from the previous
version, shown in Table~\ref{tab:alf}. Its comprised by a four column
...
\begin{table}
 \caption{The output of \ty{alfy} for a window of 5 bp.}\label{tab:alf}
    \begin{center}
      \begin{tabular}{rrrr}
        \hline
        \multicolumn{4}{l}{\textgreater q1.fasta}\\
        1 & 5 & 60 & s1\\
        6 & 10 & 80 & s2\\
 \multicolumn{4}{l}{\textgreater q2.fasta}\\
 1 & 10 & 90 & s2\\\hline
      \end{tabular}
      \end{center}
\end{table}
