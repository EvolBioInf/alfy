...
suffix array, $\mbox{esa}$ (line 2). Then we initialize the matching
start in $q$, $j$, to 1 (line 3). With these preparations out of the
way, we iterate across $q$ and calculate an array of longest match
lengths, $\mbox{ml}$, and an array of subjects inducing these lengths,
$\mbox{su}$.
...
position beyond the terminating mismatch, $j\leftarrow 1+\ell+1$ (line
10) and the analysis repeats.
