...
void getLcpTreeShulens(FILE *fpout, Args *a, SequenceUnion *seqUnion, FILE *fwout) {
  ...
  traverseLcpTree(lcpTab, sa, seqUnion->seqUnion, seqUnion->numOfSubjects,
		  seqUnion->numOfQueries, seqUnion->seqBorders, leftBorders,
		  strandBorders, &listQueryIntervalsFwd, &listQueryIntervalsRev,
		  maxDepth, maxShulens, fastSearch, lastIndex, &root);
...
void determineQS(Interval *interval, Int64 numOfSubjects, Int64 *seqBorders, Int64 *leftBorders, short *subjects, int step, Int64 *queryLeaves, 
  ...
  } else { /* an internal node of lcp-interval tree */
