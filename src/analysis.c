...
qNode ***windowAnalysis(Args *args, SequenceUnion *seq, FILE *fwout, queryInterval **listQueryIntervalsFwd, Int64 *strandBorders, Int64 *leftBorders, qNode **root, int binsearch, Int64 *minSumWin, FILE *fpout) {
  ...
  for (iQuery = 0; iQuery < seq->numOfQueries; iQuery++) {
	...
	fprintf(fpout, "\nWindows analysis\n");
      ...
      queryWinAnalysisNew(seq, fwout, root[iQuery], strandBorders[iQuery] - leftBorders[iQuery], winInc, winLen,
			  ...
			  stepSeg, segSize, nSeg, minSumWin[iQuery], s, subjStartPos, subjEndPos, args->r,
			  list, (int)iQuery, fpout, numOfElem, args->f, args);
...
void queryWinAnalysisNew(SequenceUnion *seq, FILE *fwout, qNode *p, Int64 leftBorder,
			 Int64 incLen, Int64 winSize, Int64 *winStart, Int64 *winEnd,
			 Int64 *startSeg, Int64 *endSeg, long long int *sums,
			 long long int **w, Int64 *i, Int64 *removeSeg, int *computeSum,
			 Int64 stepSeg, Int64 segSize, Int64 nSeg, Int64 minSumWin,
			 float *s1, Int64 *subjStartPos, Int64 *subjEndPos,
			 int printRunnerUp, sNode *list, int iQuery, FILE *fpout,
			 Int64 maxNumOfElem, int minFragmentLen, Args *args) {
  ...
  while (1) {
	...
	for (j = 0; j < seq->numOfSubjects; j++) { /* j and k --> to cover all subjects */
	  if (s.subjectIndex[j / WORDSIZE] & (MASK_ONE << (j % WORDSIZE))) { // if the k-th bit of j-th word is set -->	(k + 1 + j * WORDSIZE)-subject is winner for p						
	    ...
	    ++ cntIntervalSubjects; 
		   ...
		   && subjStartPos[j] <= *endSeg) { /* if j-th subject is not winner for p, but contributes to some smaller extent from some previous interval, 
	...
	*removeSeg = 0; // index of the sum of a segment to be subtracted in the next iteration
      } else if (*computeSum == 1) { /* after first window - compute sum using the previous one;
				    * add just new segment (this doesn't have to be the sum of the next window)*/
	...
	if (!SIMDEBUG) {
	  ...
	  printWinMaxRunnerUp(*winStart, *winEnd, winSize, incLen, maxAvgSl, runnerUpAvgSl,
			      minSumWin, seq, sums, fwout, printRunnerUp, &an, &prevAn,
			      &currAn, lastWindow, fpout, list, &prev, slist, args);
	  if (lastWindow == 1) {
	    break;
	  }
	}
	...
	*removeSeg = (*removeSeg + 1) % nSeg; // index of the sum to be subtracted in the next iteration				
    ...
    } else {  /* same segment, but get next interval */
      ////// !!!
      ++ k;
      if (k >= maxNumOfElem) {
        computeLastAnnotation(computeSum, i, sums, w, seq->numOfSubjects, winEnd, winStart, s.rb, &maxAvgSl, &runnerUpAvgSl);
...
void formIntervalList(qNode *p, Int64 leftBorder, sNode **list, Int64 *numOfElem,
		      SequenceUnion *seq, Int64 iQuery, Int64 *subjectIndex,
		      double **m_pvalue, Int64 *maxNumOfElem) {
...
void formIntervalListI(qNode *p, Int64 leftBorder, sNode **list, Int64 *numOfElem, SequenceUnion *seq,
		       Int64 iQuery, Int64 *subjectIndex, double **m_pvalue, Int64 *maxNumOfElem) {
...
void printWinMaxRunnerUp(Int64 winStart, Int64 winEnd, Int64 winSize, Int64 winInc, long long int maxAvgSl, long long int runnerUpAvgSl, Int64 minSumWin
  ...
  if (fpout) {
    ...
    if (minSumWin >= maxAvgSl) { // weak signal --> print "winners" in square brackets
      fprintf(fpout, "[");
    }
  }	
  for (j = 0; j < seq->numOfSubjects; j++) {
  ...
  if (winEnd == winSize - 1) {
    ...
    *currAn = *an;
    ...
    } else {     // extend current annotation
  ...
  if (!(*computeSum)) {
    *computeSum = 1;
	...
	sums[j] += w[l][j]; /* sum for a window for the subject j */
				  ...
				  * add just new segment (this doesn't have to be the sum of
				  * the next window)*/
...
void computeLastAnnotation(int *computeSum, Int64 *i, long long int *sums, long long int **w,
			   Int64 numOfSubjects, Int64 *winEnd, Int64 *winStart, Int64 s_rb,
