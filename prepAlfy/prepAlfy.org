...
#+end_src
#+begin_export latex
We first create a map between the attributed numerical subject ID and
the subject filename. Then to find the maximal subject length, we
iterate over the subject files and for each file iterate over the
sequences it contains to find the longest subject file. We also fill
in the subject ID with the corresponding subject filename.
#+end_export
...
#+end_export
#+begin_src go <<Structures, Ch. \ref{ch:prep}>>=
  type Matches struct {
	  matchLengths [][]int
	  subjectID [][]int
  }
...
increament the counter of working goroutines and construct the
...
#+end_src
#+begin_export latex
We pass the current set of subject names to the goroutine. Inside it
we defer it closure. We also declare and allocate space for holding
the matches lengths and subject IDs, stream the query against the
subjects and pass the matches along the channel.
#+end_export
...
#+begin_src go <<Allocate space for match lengths and subject ID, Ch. \ref{ch:prep}>>=
  for _, querySeq := range querySeqs {
	  ...
	  matches.subjectID = append(matches.matchLengths, lengths)
...
#+end_src
#+begin_export latex
In the iterator we pick the matches structure with the results from
