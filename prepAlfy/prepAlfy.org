...
space for the array of matches lengths and another for the array of
subject IDs.
#+end_export
#+begin_src go <<Allocate space for match lengths and subject ID, Ch. \ref{ch:prep}>>=
  for _, querySeq := range querySeqs {
	  ...
	  //<<Stream against subject files, Ch. \ref{prep}>>
  }
  f.Close()
#+end_src
#+begin_export latex
We then iterate and read the subject files. After streaming the files
we write the matches lengths to a file.
#+end_export
#+begin_src go <<Stream against subject files, Ch. \ref{prep}>>=
...
#+begin_src go <<Read subject files, Ch. \ref{ch:prep}>>=
  ...
  for sc.ScanSequence() {
	  ...
	  //<<Calculate enhanced suffix array, Ch. \ref{prep:ch}>>
...
sequences.
#+end_export
#+begin_src go <<Calculate enhanced suffix array, Ch. \ref{prep:ch}>>=
  d = bytes.ToUpper(d)
  e := esa.MakeEsa(d)
#+end_src
#+begin_export latex
