...
store the corresponding subject IDs. This is done with the \ty{Store}
...
IDs are converted to 1-based values to match the header. When printing
the subject filenames we also remove the extension.
#+end_export
#+begin_src go <<Write matches and subjects, Ch. \ref{ch:prep}>>=
  for j := 0; j < len(ml); j++ {
	  str := make([]string, len(ids[j]))
	  if *optN {
		  //<<Print subject filenames, Ch. \ref{ch:prep}>>
	  } else {
  		  //<<Print subject ids, Ch. \ref{ch:prep}>>
	  }
	  fmt.Printf("%d\t%s\n", ml[j],
		  strings.Join(str,","))
  }
#+end_src
#+begin_export latex
To print the subject filenames we iterate over the range of existing
IDs, and convert the numerical id to a string. Since the results are
on a 1-based scale we add plus one to the numerical id. Then we clean
the filename to remove the extension, and create a slice of names to
be printed.
#+end_export
#+begin_src go <<Print subject filenames, Ch. \ref{ch:prep}>>=
...
#+end_src
#+begin_export latex
To print the numerical ids we iterate over the range of values present
in ids and group it into a single slice of numerical ids ready to
print. We also convert the ids into a 1-based notation by adding plus
one to every id to be printed.
#+end_export
#+begin_src go <<Print subject ids, Ch. \ref{ch:prep}>>=
  for i, val := range ids[j] {
