...
goroutine. Inside it we defer its closure. We also declare and
allocate space for holding slices of matches lengths and subject
IDs. Then we stream the query against the esas and pass the matches
along the channel. 
#+end_export
...
each the same length as the respective sequence.
#+end_export
...
are stored in a two-dimensional master set of lengths.
#+end_export
...
We start by determining the list of subject IDs that are present in
the results and then print it out.
#+end_export
...
#+end_src
#+begin_export latex
We create a map to save the subject IDs that we detect. Then iterate
over the range of existings ids, and if the id was not previously seen
...
#+end_src
#+begin_export latex
Then we go over the subject names, and for every subject name present
in the map of seen subjects we print out the corresponding key,
numeric id, and string, the filename. For more readability we convert
the subject ids to 1-based.
#+end_export
