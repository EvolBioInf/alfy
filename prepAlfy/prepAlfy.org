...
allocate space for holding the matches lengths and subject IDs, stream
the query against the esas and pass the matches along the channel.
...
#+begin_src go <<Allocate space for match lengths and subject ID, Ch. \ref{ch:prep}>>=
  for i, querySeq := range querySeqs {
	  ...
	  //<<Stream against esas, Ch. \ref{ch:prep}>>
  }
#+end_src
#+begin_export latex
We then iterate over the esas, and calculate the matches lengths
between the current query and the set of esas in the goroutine.
#+end_export
#+begin_src go <<Stream against esas, Ch. \ref{ch:prep}>>=
