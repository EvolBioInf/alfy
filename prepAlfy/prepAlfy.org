...
followed by pairs of match lengths and sequence
identifiers. Figure~\ref{fig:out} shows an example of the
\ty{prepAlfy} output.
#+end_export
#+begin_export latex
...
also allow the user to print subject names instead of identifiers. To
help debugging the program, the user can chose to print the raw table
of position annotations instead of the default table of winning
intervals. Also for debugging purposes, the user can step through all
possible match start positions instead of skipping just beyond the
mismatch that terminates the current match.
#+end_export
...
longest subject file. We also keep track of the total length of all
subject sequences, and calculate the GC-content. For this we create
new variables to hold the total subject length, the GC content, and
the set of nucleotides [CGcg] to be counted. The GC counted is
calculated after iterating over all the suject sequences. Since the
subject statistics are set, we print it only once before starting the
#+end_export
#+begin_src go <<Find maximum subject length, Ch. \ref{ch:prep}>>=
  ...
  //<<Print subject statistics, Ch. \ref{ch:prep}>>
#+end_src
#+begin_export latex
After iterating over all the subject sequences, we can calculate the
final GC-content and print it, together with the cumulative sequence
length. Since the set of subject IDs are static, it only needs to be
printed once.
#+end_export
#+begin_src go <<Print subject statistics, Ch. \ref{ch:prep}>>=
  gc = float64(g)/float64(slen)
  fmt.Printf("-Stats\t%d\t%f\n", slen, gc)
#+end_src
#+begin_export latex
We use a scanner to iterate over the sequences in the current neighbor
file. We store the sequence data in the variable d, and its length in
the variable l. We then calculate the statistics of the set of subject
sequences, and store the length of the the longest subject sequence.
#+end_export
#+begin_src go <<Iterate over subjects, Ch. \ref{ch:prep}>>=
  ...
  for sc.ScanSequence() {
	  d := sc.Sequence().Data()
	  l := len(d)
	  //<<Get subject statistics, Ch. \ref{ch:prep}>>
	  if l > msl {
		  msl = l
	  }
  }
...
#+end_src
#+begin_export latex
To calculate the cumulative subject length, we just add the length of
all subject files. To get the GC-content, we also need the total count
of all G and C bases.
#+end_export
#+begin_src go <<Get subject statistics, Ch. \ref{ch:prep}>>=
  slen = slen + l
  for i ,_ := range d {
...
store the corresponding subject IDs. This is done with the \ty{Update}
function of the mat package.
#+end_export
...
loop variables. This is done with the \ty{Interpolate} function of the
mat package.
#+end_export
...
printing the statistics of the subject sequences: the cumulative
sequence length and the respective GC-content. Only then do we print
the query filename without the extension and iterate over the query
...
#+begin_src go <<Write matches to stdout, Ch. \ref{ch:prep}>>=
  ...
  for i, querySeq := range querySeqs {
	  ...
	  if *optR {
		  //<<Write raw table, Ch. \ref{ch:prep}>>
	  } else {
		  //<<Write intervals table, Ch. \ref{ch:prep}>>
	  }
  }
#+end_src
#+begin_export latex
The header consists of a single line that starts with a hash followed
by the name of the query file and its length, followed by a key/value
map to the subject id. Unless the user requested subject names, the
header will contain a legend consisting of pairs of subject ID and
subject file linked by ``='', that is,
\begin{verbatim}
...
#+end_src
#+begin_export latex
We write the raw table of positions, match lengths, and annotations.
#+end_export
#+begin_src go <<Write raw table, Ch. \ref{ch:prep}>>=
  for j := 0; j < len(ml); j++ {
	  fmt.Println(j, ml[j], ids[j])
  }
#+end_src
#+begin_export latex
Then we write the summary table containing the interval of matches
found and their respective subject IDs. The output table consists of
four columns, start and end of interval, the match length at the start
of the interval and the corresponding subject IDs. A new interval is
printed at the start of every match or whenever the slice of subject
IDs changes.

We start by initializing three arrays to store match coordinates as
well as match lengths.
#+end_export
#+begin_src go <<Write intervals table, Ch. \ref{ch:prep}>>=
  ...
  mat[0] = ml[0]
  row := 0
  //<<Get interval coordinates, Ch. \ref{ch:prep}>>
#+end_src
#+begin_export latex
To get the interval coordinates we iterate over the length of the ml
array and for every position we keep track of the match length of the
current and previous element. We also want to check if we have been
requested to print the subject filenames or just the numeric IDs. If
the match length of the current position is larger than from the
previous one we have found a new interval. The same applies if the
slice of subject IDs changes composition. At the end we close the last
interval to be printed.
#+end_export
#+begin_src go <<Get interval coordinates, Ch. \ref{ch:prep}>>=
  for j := 1; j < len(ml); j++ {
	  prev := ml[j-1]
	  curr := ml[j]
	  pId := ids[j-1]
	  slices.Sort(pId)
	  slices.Sort(ids[j])
	  //<<Check if printing filenames or ids, Ch. \ref{ch:prep}>>
	  if curr > prev ||
		  !slices.Equal(pId,ids[j]) {
		  //<<Print table intervals, Ch. \ref{ch:prep}>>
	  }
	  if j == len(ml)-1 {
		  //<<Print last interval, Ch. \ref{ch:prep}>>
	  }
  }
#+end_src
#+begin_export latex
If the user requested that the subject filenames be printed, then we
map the numeric IDs to the subject filenames present in the
subjectNames array. We remove the last extension from the filename and
then we join the elements together into a single string. Otherwise, we
just join the numeric IDs together into a single string. We want the
result to be 1-based hence why we add plus one to the numeric ID.
#+end_export
#+begin_src go <<Check if printing filenames or ids, Ch. \ref{ch:prep}>>=
  str:= make([]string, len(pId))
  if *optN {
	  for i, val := range pId {
		  name := strconv.Itoa(val+1)
		  name = subjectNames[val]
		  //<<Clean subject name extension, Ch. \ref{ch:prep}>>
...
#+end_src
#+begin_export latex
To trim the extension of the subject filenames we split the name into
several substrings using the period as a delimiter. Then we drop the
last substring.
#+end_export
#+begin_src go <<Clean subject name extension, Ch. \ref{ch:prep}>>=
...
#+end_src
#+begin_export latex
To print the intervals we set the end of the interval to the previous
position, then we print the stored coordinates of the start, match and
the stored slice of previous subjectIDs. Then we increment the row
counter, and update the current position as the start of the new
interval with the respective match length.
#+end_export
#+begin_src go <<Print table intervals, Ch. \ref{ch:prep}>>=
  end[row] = j-1
  fmt.Printf("%d\t%d\t%d\t%v\n",
	  start[row],end[row],
...
#+end_src
#+begin_export latex
After iterating over the match lengths array we close the last opened
interval. For this we print the current position as the starting
element, the length of the array as the (non-inclusive) end of the
interval, the corresponding match length at the start of the interval
and the slice of subject IDs.
#+end_export
#+begin_src go <<Print last interval, Ch. \ref{ch:prep}>>=
  fmt.Printf("%d\t%d\t%d\t%v\n",
	  start[row],len(ml)-1,
	  mat[row],strings.Join(str,","))
#+end_src
...
#+end_src
#+begin_export latex
In our third, we use subject names instead of IDs.
#+end_export
#+begin_src go <<Construct tests, Ch. \ref{ch:prep}>>=
...
#+end_src
#+begin_export latex
In our final test, we check the stepping flag.
#+end_export
#+begin_src go <<Construct tests, Ch. \ref{ch:prep}>>=
